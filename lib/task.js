...
import isThenable from './util/thenable-is.js';
import callThenable from './util/thenable-call.js';
import './util/array-replace.js';
...
var Task = proto.extend({
	...
	afterFailure(){
		this.notify('fail', this.value);
		this.propagateState();
		this.clean();
	},

	afterCompletion(){
		this.notify('complete', this.value);
		this.propagateState();
		this.progress(this);
		this.clean();
	},
	...
	// comme l'état de la tâche a été modifié
	// toutes les tâches adapte leur propre état en fonctinon 
	propagateState(){
		this.propagate('adoptState', 'both', this);
	},
	...
	adoptState(task){
		// task state is already set, make it the state of this task
		if( task.isCancelled ){
		...
		// of making this adopt the state of task
			...
			if( this.isPaused ){
				this.debug(this, 'resumed by', task);
				this.resume();
			}

			this.debug(this, 'state has a dependency to', task);
		}
	},

	become(task, immediatly){
